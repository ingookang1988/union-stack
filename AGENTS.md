...
2. **Respect the permission tiers.** Never edit Schema-tier files (`.union-stack/monocron`, `topology`, `roadmap`, `plan`, `contracts`). They are read-only to you. Wiki-tier files (`feature`, `sprint`, `mechanism/derived`) get **row-level atomic edits only — never full-file rewrites**. Raw-tier (`mechanism/raw`, `archive_ledger.md`) is append-only and system-driven.
...
1. `.union-stack/monocron/` — what this project is (identity, scope boundary, domain vocabulary).
