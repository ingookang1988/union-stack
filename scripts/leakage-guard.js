...
const METHODOLOGY = new Set([
  ...
  'TOOL-24_lineage_tree.md', 'TOOL-25_dashboard.md', 'TOOL-26_adopter_arm.md',
  'TOOL-27_ci_container.md', 'TOOL-28_test_catalog.md',
  ...
  // 상류 자신의 테스트 인프라 카탈로그([PRO-21] 1단계 도그푸딩). 배송되는 도구(scripts/*.test.js
  // 관용구·공유 픽스처)의 호출 규약이라 TOOL 카드와 같은 클래스 — 위탁 데이터 0, 더미化하면
  // 카탈로그가 카탈로그이기를 그만둔다. 어답터에선 init 이 파일째 제거하므로 노출면도 없다.
  'CON-00_test_infra_catalog.md',
]);
