...
## 4. 미해결 / 주의
- **회전 주기가 짧다(관측 대상)**: 항목당 ~2KB 라 [PRO-18]의 60%(18KB) 목표는 헤드룸 5건뿐이다
  (16.3 → 이미 25.6KB). **두 번째 회전 때 60% 조정 여부를 판정**하라(§6 자동화 논의와 같은 자리).
...
- **[TOOL-25] 카드 4KB 상시 밀착**: 축이 더 늘면 축별 분할(`TOOL-25a`).
- **[PRO-17] 승인 대기**: 시뮬레이터가 그 팔의 수동판이고 이번에 값을 증명했다. 승인 시
  `harness.yml` 잡으로 고정 — 단 시뮬레이터가 스크래치패드에만 있어 **스크립트화가 선행**이다.
