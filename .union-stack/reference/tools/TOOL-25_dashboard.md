...
## 시간축 페이지
격자 §3.2가 식별한 **두 번째 빈칸**이 이 축이다 — "같은 실수의 반복"은 여러 스프린트에 걸쳐서만
보이고 어느 단일 스냅샷에도 나타나지 않는다(E1 실측: 페이오프 최대 구간이 비국소 지식 — [ADR-04]가
이 평면의 제거를 금지). 세 층을 편다: **결정 밀도**(날짜별 ADR 막대) · **재제안 차단**([PRO-14] —
차단 대상과 재개 조건, 최근 결정 목록에도 ⛔ 표식) · **오답노트 LSN**(반복 횟수·유효 사유) ·
**HISTORY 전략 분기점**(사실+근거 쌍, 예시 행은 흐리게). 파서는 소유 도구의 export 소비:
`parseLedger`([TOOL-22]) · `parseEntries`(history-linter) — 중복 파서는 드리프트라 각 정본에 신설했다.
